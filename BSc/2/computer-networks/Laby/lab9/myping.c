...
void send_ping_request(int sock, char *s_send_addr) {
  ...
  gettimeofday(&current_time, NULL);
   ...
   long long timestamp = current_time.tv_sec * 1000000 + current_time.tv_usec;

  data_len = snprintf(((char *) send_buffer + ICMP_HEADER_LEN),
                      sizeof(send_buffer) - ICMP_HEADER_LEN,
                      "%lld",timestamp);
  if (data_len < 1)
    syserr("snprintf");
...
int receive_ping_reply(int sock, int seq) {
  ...
  long long got_time = current_time.tv_sec * 1000000 + current_time.tv_usec;
...
//  printf("received %zd bytes from %s\n", len, inet_ntoa(rcv_addr.sin_addr));
  ...
  char * time_str= (rcv_buffer+ip_header_len+ICMP_HEADER_LEN);
  long long request_time = atoll(time_str);

  float t = (float)(got_time - request_time) / 1000;
  char host[16];
  inet_ntop(AF_INET, &(ip->ip_src.s_addr), host, INET_ADDRSTRLEN);

  int ttl = ip->ip_ttl;

  printf("%zd bytes from %s icmp_seq=%d ttl=%d time=%.3f ms\n", data_len, host, seq, ttl, t);
  return 1;
}
int sock;
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    while (!receive_ping_reply(sock, i));
