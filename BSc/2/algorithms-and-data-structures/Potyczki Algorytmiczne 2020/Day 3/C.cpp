...
int main() {
    ...
    vector<vector<int>> dostawczaki(maxW * 2, {0, 0});


    FOR(0, n) {
        int r = input_unsigned_number();
        int w = input_unsigned_number();
        int t = input_unsigned_number();

        int new_w = w - t + maxW;
        dostawczaki[new_w][r - 1]++;
    }

    int res = 0;

    FOR(0, maxW * 2) {
        res += min(dostawczaki[i][0], dostawczaki[i][1]);
    }
