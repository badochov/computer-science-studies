...
let test q l =
  try
    let b, nq =
      List.fold_left
        (fun a x ->
          let e, nq = delete_min (snd a) in
          if compare x e != 0 then raise WA else (true, nq))
        (true, q) l
    in
    b && is_empty nq
  with WA -> false
...
let rec add_lots f a l iter =
  if iter = 0 then a
  else add_lots f (List.fold_left (fun q x -> f x q) a l) l (iter - 1)

(* Size of a queue *)
let size q =
  let rec aux q a =
    if is_empty q then a else aux (snd (delete_min q)) (a + 1)
  in
  aux q 0
...
let q1 =
  empty |> add 2.3 |> add 1.1 |> add 3.2 |> add 0.01 |> add 222.1 |> add 42.42
  |> add 1.03
...
let l7 =
  List.sort compare (add_lots app [] [-3.1; 22.1; -43.2; 115.0; 8.8] 20000)
...
let q1 =
  empty |> add "abba" |> add "acca" |> add "baba" |> add "abbabb"
...
let l1 =
  List.sort compare
    ["abba"; "acca"; "baba"; "abbabb"; "aabbab"; "aaabbaa"; "1.23"]
...
let l7 =
  List.sort compare
    (add_lots app [] ["lorem"; "ipsum"; "dolor"; "sit"; "amet"] 20000)
