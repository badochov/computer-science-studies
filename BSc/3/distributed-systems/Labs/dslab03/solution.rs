...
impl FibonacciModule {
    ...
    pub(crate) fn create(
        ...
        queue
            .clone()
            .send(FibonacciSystemMessage::RegisterModule(FibonacciModule {
                num: initial_number,
                limit,
                id,
                other: None,
...
fn run_executor(rx: Receiver<FibonacciSystemMessage>) -> JoinHandle<()> {
    ...
    thread::spawn(move || {
        while let Ok(msg) = rx.recv() {
            match msg {
                FibonacciSystemMessage::RegisterModule(module) => modules.push(module),
                FibonacciSystemMessage::Init { id, other } => {
                    get_module(&mut modules, id).init(other)
                }
                FibonacciSystemMessage::Message { id, idx, num } => {
                    get_module(&mut modules, id).message(idx, num)
                }
                FibonacciSystemMessage::Done => return,
            }
        }
    })
}

fn get_module(modules: &mut std::vec::Vec<FibonacciModule>, id: Ident) -> &mut FibonacciModule {
    for module in modules {
        if module.id == id {
            return module;
        }
    }
    panic!("Module with given id not found");
}
