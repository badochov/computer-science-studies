...
__global__ void transpose(float *odata, const float *idata)
{
  ...
  {
    block[threadIdx.x * TILE_DIM + threadIdx.y] = idata[(y + j) * width + x];
  }

  __syncthreads();
  
  #define PERTHREAD (TILE_DIM / BLOCK_ROWS)
  for (int j = 0; j < PERTHREAD; j++)
  {
    odata[x * TILE_DIM + (y * PERTHREAD + j)] = block[threadIdx.x * TILE_DIM + threadIdx.y * PERTHREAD + j];
  }
} 
